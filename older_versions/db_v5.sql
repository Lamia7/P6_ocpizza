...
-- Modifs faites : suppression de utf8 mb4-bin et VISIBLE,
...
CREATE INDEX `by_name_idx` ON `ocpizza`.`purchase_status` (`name` ASC);
...
CREATE TABLE IF NOT EXISTS `ocpizza`.`purchase` (
  ...
  `address_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`, `client_id`, `restaurant_id`, `purchase_status_id`, `payment_method_id`, `delivery_method_id`, `address_id`),
  CONSTRAINT `fk_purchase_client1`
    FOREIGN KEY (`client_id`)
    REFERENCES `ocpizza`.`client` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_purchase_restaurant1`
  ...
  CONSTRAINT `fk_purchase_payment_method1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_purchase_delivery_method1`
    FOREIGN KEY (`delivery_method_id`)
    REFERENCES `ocpizza`.`delivery_method` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_purchase_address1`
    FOREIGN KEY (`address_id`)
    REFERENCES `ocpizza`.`address` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `by_purchase_status_idx` ON `ocpizza`.`purchase` (`purchase_status_id` ASC);

CREATE INDEX `by_payment_method_idx` ON `ocpizza`.`purchase` (`payment_method_id` ASC);

CREATE INDEX `by_delivery_method_idx` ON `ocpizza`.`purchase` (`delivery_method_id` ASC);

CREATE INDEX `by_address_idx` ON `ocpizza`.`purchase` (`address_id` ASC);

CREATE INDEX `by_date_idx` ON `ocpizza`.`purchase` (`date` ASC);
